            ...
            <table role="presentation" cellspacing="0" cellpadding="0" border="0" width="100%" style="max-width: 600px; margin: 0 auto; background-color: #fff; border-radius: 10px; box-shadow: rgba(0, 0, 0, 0.16) 0px 1px 4px;">
                <tr>
                    <td style="padding: 20px;">
                        <h2 style="margin-bottom: 20px;">{{ __('NEW WITHDRAWAL REQUEST') }}</h2>
                        <p>{{ __('Dear Administrator,') }}</p>
                        <p>{{ __('We wish to inform you that user') }} <strong>{{ $usuario }}</strong> {{ __('has just requested a withdrawal of') }} <strong>{{ $valor }}</strong> {{ __('on our platform.') }}</p>
                    </td>
                </tr>
            </table>
        </td>
